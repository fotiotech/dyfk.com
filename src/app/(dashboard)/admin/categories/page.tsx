...
import { deleteCategory, getCategory, createCategory } from "@/app/actions/category";
...
const Categories = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const images = imageUrl?.length! > 1 ?imageUrl: imageUrl?.[0];
        ...
        <div className="lg:flex gap-3 mb-2">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="categoryName"
              value={editId ? categoryEdit.categoryName : categoryData.categoryName}
              onChange={handleCategoryData}
              className="w-full bg-[#eee] dark:bg-sec-dark"
            />
          </div>
        </div>
        <FilesUploader files={imageUrl} setFiles={setImageUrl} />
          ...
          <ul className="flex flex-col gap-1 max-h-96 overflow-y-auto scrollbar-none">
            {categories &&
              categories.map((cat) => (
                ...
                >
                  <span onClick={() => setCatId(cat._id as string)} className="flex-1 hover:text-sec hover:bg-opacity-5 p-1">
                    {cat.categoryName}
                  </span>
                  <div className="flex gap-2">
                    <span onClick={() => setEditId(cat._id  as string)} className="border px-1 hover:text-sec hover:bg-opacity-5 p-1">
                      Edit
                    </span>
                    <span
